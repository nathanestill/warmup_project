#!/usr/bin/env python3
import rospy
from geometry_msgs.msg import Twist, Vector3, Point, Quaternion, TwistStamped
...
class PersonfollowState(object):
	"""docstring for PersonfollowState"""
	def __init__(self):
		rospy.init_node("PersonfollowState")
		self.pub = rospy.Publisher("/cmd_vel_state",TwistStamped,queue_size=10)
	...
	def goToPerson(self):
		if(self.Person.x < 0.3 or abs(self.Person.y) > 1.3 or self.Person.x > 3.0):
			self.pub.publish(TwistStamped(header=Header(frame_id="personfollow"),twist=Twist()))
		else:
			self.pub.publish(TwistStamped(header=Header(frame_id="personfollow"),twist=Twist(linear=Vector3(x=numpy.clip(self.Person.x,-1.5,1.5)),angular=Vector3(z=numpy.clip(self.Person.y,-1.5,1.5)))))
